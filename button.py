...
async def async_setup_entry(
    ...
    def _add_new_button_entities() -> None:
        entities = []
        ...
        for entity_schema in supervisor_entities:
            try:
                if entity_schema.get("type") != "button":
                    continue
        ...
        if entities:
            _LOGGER.info("Adding %d new button entities from updated schema", len(entities))
        ...
        for registry_entry in er.async_entries_for_config_entry(registry, entry.entry_id):
            ...
            if unique_id in desired_unique_ids:
                continue
            _LOGGER.info("Pruning stale button entity %s", registry_entry.entity_id)
        ...
        if added_by_service:
            _LOGGER.warning("Button entity sync added per service: %s", dict(added_by_service))
        if pruned_by_service:
            _LOGGER.warning("Button entity sync pruned per service: %s", dict(pruned_by_service))

        known_unique_ids.clear()
        known_unique_ids.update(desired_unique_ids)

    _add_new_button_entities()
    entry.async_on_unload(coordinator.async_add_listener(_add_new_button_entities))
