#!/usr/bin/env python3
"""Standalone Wildlife Monitor dashboard service (port 8094 by default)."""
...
logger = logging.getLogger("perimetercontrol.wildlife_dashboard")

SUPERVISOR_API_BASE = os.environ.get("PERIMETERCONTROL_SUPERVISOR_API", "http://127.0.0.1:8080/api/v1")
DASHBOARD_PORT = int(os.environ.get("PERIMETERCONTROL_WILDLIFE_DASHBOARD_PORT", "8094"))
...
class DashboardHandler(tornado.web.RequestHandler):
    def get(self) -> None:
        ...
        self.write(
            f"""<!doctype html>
<html><head><meta charset='utf-8'><title>Wildlife Dashboard</title>
...
<h1>Wildlife Monitor Dashboard</h1>
    ...
    def get(self) -> None:
        try:
            ...
            wildlife_entities = [
                ...
                and (entity.get("capability") == "wildlife_monitor" or entity.get("capability_id") == "wildlife_monitor")
            ]
            self.write({"entities": wildlife_entities})
        except Exception as exc:
            logger.warning("Failed to fetch wildlife entities: %s", exc)
            self.set_status(502)
...
if __name__ == "__main__":
    ...
    logger.info("Wildlife dashboard listening on 0.0.0.0:%s", DASHBOARD_PORT)
