#!/usr/bin/env python3
"""Standalone ESL Access Point dashboard service (port 8092 by default)."""
...
logger = logging.getLogger("perimetercontrol.esl_dashboard")

SUPERVISOR_API_BASE = os.environ.get("PERIMETERCONTROL_SUPERVISOR_API", "http://127.0.0.1:8080/api/v1")
DASHBOARD_PORT = int(os.environ.get("PERIMETERCONTROL_ESL_DASHBOARD_PORT", "8092"))
...
class DashboardHandler(tornado.web.RequestHandler):
    def get(self) -> None:
        ...
        self.write(
            f"""<!doctype html>
<html><head><meta charset='utf-8'><title>ESL Dashboard</title>
...
<h1>ESL Access Point Dashboard</h1>
    ...
    def get(self) -> None:
        try:
            ...
            esl_entities = [
                ...
                and (entity.get("capability") == "esl_ap" or entity.get("capability_id") == "esl_ap")
            ]
            self.write({"entities": esl_entities})
        except Exception as exc:
            logger.warning("Failed to fetch ESL entities: %s", exc)
            self.set_status(502)
...
if __name__ == "__main__":
    ...
    logger.info("ESL dashboard listening on 0.0.0.0:%s", DASHBOARD_PORT)
