#!/usr/bin/env python3
"""
GPIO Control Bokeh Dashboard
Serves the GPIO dashboard at the same port as the old Tornado dashboard (default: 8095).
"""
import os
import logging
...
from gpio_bokeh_layouts import create_gpio_dashboard_layout
from callbacks import setup_gpio_callbacks
...
DASHBOARD_LOG_FILE = os.path.join(LOG_ROOT, 'gpio_dashboard.log')
...
def create_app(doc):
    ...
    layout, widgets = create_gpio_dashboard_layout(data_manager)
    doc.add_root(layout)
    for key, value in widgets.items():
        setattr(doc, key, value)
    setup_gpio_callbacks(doc, data_manager)
...
if __name__ == "__main__":
    ...
    logger.info(f"GPIO Bokeh dashboard running on port {DASHBOARD_PORT}")
    server.start()
    try:
