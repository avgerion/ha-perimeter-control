"""Dynamic switch entities for Perimeter Control."""
...
async def async_setup_entry(
    ...
    """Set up switch entities from Supervisor API schema."""
    ...
    def _add_new_switch_entities() -> None:
        entities = []
        ...
        for entity_schema in supervisor_entities:
            try:
                if entity_schema.get("type") == "switch":
                    service_id = str(
                ...
                entity_id = entity_schema.get("id", "unknown_switch")
                _LOGGER.error(
                    "Failed to process switch entity '%s': %s. Continuing with other switches.",
                    entity_id,
                    exc,
                    exc_info=True,
                )
        ...
        if entities:
            _LOGGER.info("Adding %d new switch entities from updated schema", len(entities))
            async_add_entities(entities)

        registry = er.async_get(hass)
        for registry_entry in er.async_entries_for_config_entry(registry, entry.entry_id):
            if registry_entry.platform != DOMAIN or registry_entry.domain != "switch":
                continue
            ...
            if unique_id in desired_unique_ids:
                continue
            _LOGGER.info("Pruning stale switch entity %s", registry_entry.entity_id)
        ...
        if added_by_service:
            _LOGGER.warning("Switch entity sync added per service: %s", dict(added_by_service))
        if pruned_by_service:
            _LOGGER.warning("Switch entity sync pruned per service: %s", dict(pruned_by_service))

        known_unique_ids.clear()
        known_unique_ids.update(desired_unique_ids)

    _add_new_switch_entities()
    entry.async_on_unload(coordinator.async_add_listener(_add_new_switch_entities))
